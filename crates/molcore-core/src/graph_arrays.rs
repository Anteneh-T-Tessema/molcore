...
pub fn mol_to_graph_arrays<'py>(
    ...
    let e = graph.edge_count() * 2; // bidirectional

    // Node features: [atomic_num, is_aromatic, formal_charge, num_hs]
    ...
    // Edge index (COO) + edge features (bond type one-hot)
    let mut src_col = Vec::with_capacity(e);
    let mut dst_col = Vec::with_capacity(e);
    let mut edge_feats = Array2::<f32>::zeros((e, 4));
    let mut eidx = 0usize;
    ...
    for edge in graph.edge_indices() {
        ...
        for &(a, b) in &[
            ...
            dst_col.push(b);
            for k in 0..4 {
                edge_feats[[eidx, k]] = oh[k];
            }
            eidx += 1;
        }
    }

    let mut edge_index = Array2::<i64>::zeros((2, e));
    for i in 0..e {
        edge_index[[0, i]] = src_col[i];
        edge_index[[1, i]] = dst_col[i];
    }

    Ok((
        node_feats.into_pyarray_bound(py),
        edge_index.into_pyarray_bound(py),
        edge_feats.into_pyarray_bound(py),
    ))
}
